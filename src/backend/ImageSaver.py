"""
Author: ZyMa-1
"""

import pathlib

from PySide6.QtCore import QObject
from PySide6.QtWidgets import QFileDialog

from .PathManager import PathManager


class _DialogFactory:
    @staticmethod
    def create_save_png_file_dialog(*, parent, dir: str) -> QFileDialog:
        file_dialog = QFileDialog(parent)
        file_dialog.setDefaultSuffix('png')
        file_dialog.setDirectory(dir)
        file_dialog.setFileMode(QFileDialog.FileMode.AnyFile)
        file_dialog.setAcceptMode(QFileDialog.AcceptMode.AcceptSave)
        file_dialog.setNameFilters(["PNG Files (*.png)"])
        file_dialog.setWindowTitle("Save Widget")
        return file_dialog


class ImageSaver(QObject):
    def __init__(self, parent=None):
        super().__init__(parent)

        self.PROJECT_ROOT = PathManager.PROJECT_ROOT
        self.EXPORTS_DIR = PathManager.EXPORTS_DIR

    def save_widget_to_image(self, widget, *, file_type: str = "png", parent=None) -> None | str:
        """Saves widget to an image. Returns 'None' if operation was not completed, 'filename' otherwise."""
        if file_type == "png":
            return self.save_widget_to_png(widget, parent=parent)
        else:
            raise NotImplementedError

    def save_widget_to_png(self, widget, *, parent=None) -> None | str:
        """Saves widget to a png image. Returns 'None' if operation was not completed, 'filename' otherwise."""
        file_dialog = _DialogFactory.create_save_png_file_dialog(parent=parent,
                                                                 dir=str(self.EXPORTS_DIR))

        if file_dialog.exec() == QFileDialog.DialogCode.Accepted:
            file_path = pathlib.Path(file_dialog.selectedFiles()[0])
            pixmap = widget.grab()
            pixmap.save(str(file_path))
            return file_path.name

        return None
